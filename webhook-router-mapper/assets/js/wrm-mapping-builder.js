 ...
 */
/* global jQuery, wrmData, navigator */
( function ( $, apiFetch ) {
	'use strict';
	...
	const sanitizeKey = ( s ) =>
		String( s ?? '' )
			...
			.replace( /[^a-z0-9_-]/g, '' );
	const i18n =
		window.wrmData && window.wrmData.i18n ? window.wrmData.i18n : {};
	...
	WRM.Tags = {
		...
		fetch( routeSlug ) {
			...
			apiFetch( {
				path:
					'/wrm/v1/captures?route_slug=' +
					encodeURIComponent( routeSlug ) +
					'&per_page=1',
			} )
				.then( ( captures ) => {
					...
					return apiFetch( {
						path: '/wrm/v1/captures/' + captures[ 0 ].id + '/paths',
					} );
				} )
				.then( ( paths ) => {
					if ( ! paths ) {
				...
				$cloud.html(
					'<em class="wrm-muted">' +
						esc(
							t(
								'noCaptures',
								'No captures yet — send a test request to load available tags.'
							)
						) +
						'</em>'
				);
		...
		insert( value, el ) {
			...
			el.value =
				el.value.slice( 0, start ) + value + el.value.slice( end );
		...
		showPicker( inputEl, $anchor ) {
			...
			$pop.css( {
				position: 'absolute',
				top: pos.top + $anchor.outerHeight() + 4,
				left: pos.left,
				zIndex: 99999,
			} );
			$( 'body' ).append( $pop );
	...
	WRM.BodyBuilder = ( function () {
		...
		function _fromVal( val, key ) {
			if ( val === null ) {
				return { id: _nodeUid(), type: 'null', key };
			}
			if ( typeof val === 'boolean' ) {
				return { id: _nodeUid(), type: 'boolean', key, value: val };
			}
			if ( typeof val === 'number' ) {
				return { id: _nodeUid(), type: 'number', key, value: val };
			}
			if ( typeof val === 'string' ) {
				return { id: _nodeUid(), type: 'string', key, value: val };
			}
			if ( val && val.__wrm_loop__ ) {
				return {
					id: _nodeUid(),
					type: 'array',
					key,
					loop: true,
					loop_source: val.source || '',
					loop_template: val.template || '',
					loop_join: val.join || '',
					children: [],
				};
			}
			if ( Array.isArray( val ) ) {
				return {
					...
					loop_template: '',
					loop_join: '',
					children: val.map( ( v, i ) => _fromVal( v, String( i ) ) ),
				};
			}
			return {
				id: _nodeUid(),
		...
		function parse( jsonStr ) {
			try {
				const v = JSON.parse(
					typeof jsonStr === 'string' && jsonStr ? jsonStr : '{}'
				);
			...
			switch ( node.type ) {
				...
				case 'array':
					if ( node.loop && node.loop_source ) {
						return {
							__wrm_loop__: true,
							source: node.loop_source,
							template: node.loop_template || '',
							join: node.loop_join ?? '',
						};
					}
		...
		function render( $container, root, onChange ) {
			...
			const $raw = $(
				'<textarea class="wrm-bb-raw code" rows="8" spellcheck="false">'
			)
				.val( toJSON( root ) )
				.hide();
			const $toggle = $(
				'<button type="button" class="button button-small wrm-bb-raw-toggle">⬌ Raw JSON</button>'
			);
		...
		function _renderNode( $parent, node, root, onChange, isRoot ) {
			const $row = $( '<div class="wrm-bb-row">' ).attr(
				'data-id',
				node.id
			);
			if ( ! isRoot ) {
				$row.addClass( 'wrm-bb-child' );
			}

			// Key input for object children
			if ( ! isRoot ) {
				const $key = $(
					'<input type="text" class="wrm-bb-key" placeholder="key">'
				).val( node.key || '' );
			...
			$type.on( 'change', function () {
				...
				if ( nt === 'array' ) {
					node.loop = node.loop ?? false;
					node.loop_source = node.loop_source ?? '';
					node.loop_template = node.loop_template ?? '';
					node.loop_join = node.loop_join ?? '';
				}
				if ( nt === 'string' ) {
			...
			if ( ! isRoot ) {
				const $del = $(
					'<button type="button" class="wrm-bb-del button-link" title="Remove">✕</button>'
				);
		...
		function _renderValue( $wrap, node, root, onChange ) {
			switch ( node.type ) {
				case 'string': {
					const $inp = $(
						'<input type="text" class="wrm-bb-string" placeholder="value or {{tag}}">'
					).val( node.value ?? '' );
					...
					const $tagBtn = $(
						'<button type="button" class="button-link wrm-bb-tag-btn" title="' +
							esc( t( 'insertTag', 'Insert merge tag' ) ) +
							'">⟨…⟩</button>'
					);
					$tagBtn.on( 'click', function () {
						WRM.Tags.showPicker( $inp[ 0 ], $inp );
					} );
				...
				case 'number': {
					const $inp = $(
						'<input type="number" class="wrm-bb-number">'
					).val( node.value ?? 0 );
					$inp.on( 'input', function () {
						node.value = Number( $( this ).val() );
						onChange();
					} );
					$wrap.append( $inp );
					break;
				...
				case 'boolean': {
					const $sel = $( '<select class="wrm-bb-bool">' )
						.append(
				...
				case 'object': {
					...
					const $add = $(
						'<button type="button" class="button-link wrm-bb-add">+ field</button>'
					);
				...
				case 'array': {
					...
					const $loopChk = $(
						'<label class="wrm-bb-loop-label"><input type="checkbox" ' +
							( node.loop ? 'checked' : '' ) +
							'> Loop over array</label>'
					);
					$loopRow.append( $loopChk );
					$wrap.append( $loopRow );

					const $loopCfg = $(
						'<div class="wrm-bb-loop-cfg">'
					).toggle( !! node.loop );
					const $staticCfg = $(
						'<div class="wrm-bb-static-cfg">'
					).toggle( ! node.loop );

					// Loop config
					const $src = $(
						'<input type="text" class="widefat wrm-bb-loop-src" placeholder="payload.items (dot-path)">'
					).val( node.loop_source || '' );
					$src.on( 'input', function () {
						node.loop_source = $( this ).val();
						onChange();
					} );
					const $tpl = $(
						'<input type="text" class="widefat wrm-bb-loop-tpl" placeholder="{{item.name}} (template per item)">'
					).val( node.loop_template || '' );
					$tpl.on( 'input', function () {
						node.loop_template = $( this ).val();
						onChange();
					} );
					const $join = $(
						'<input type="text" class="short-text wrm-bb-loop-join" placeholder="join or ARRAY">'
					).val( node.loop_join || '' );
					$join.attr(
						'title',
						'Use ARRAY to output a JSON array, or a string delimiter like comma to join as string'
					);
					$join.on( 'input', function () {
						node.loop_join = $( this ).val();
						onChange();
					} );
					$loopCfg.append(
						...
						$src,
						$(
							'<label>Item template (use {{item}} or {{item.field}})</label>'
						),
						$tpl,
						$( '<label>Output: ARRAY or join delimiter</label>' ),
						$join
					);
					...
					const $addItem = $(
						'<button type="button" class="button-link wrm-bb-add">+ item</button>'
					);
	...
	WRM.MappingBuilder = ( function () {
		...
		function _save() {
			if ( $config ) {
				$config.val( JSON.stringify( _state, null, 2 ) );
			}
		...
		function init( $container ) {
			...
			// Ensure arrays exist
			if ( ! Array.isArray( _state.fields ) ) {
				_state.fields = [];
			}
			if ( ! Array.isArray( _state.chains ) ) {
				_state.chains = [];
			}
			...
			if ( routeSlug && routeSlug.value ) {
				WRM.Tags.fetch( routeSlug.value );
			}
			$( document ).on( 'change', '#wrm_route_slug', function () {
				WRM.Tags.fetch( $( this ).val() );
			} );
		}
		...
		function _panel( title, $body, collapsed ) {
			...
			const $h = $( '<div class="postbox-header">' ).append(
				$( '<h2 class="hndle">' ).append(
					$( '<span>' ).text( title ),
					$(
						'<button type="button" class="handle-order-higher" aria-label="Collapse">▾</button>'
					).on( 'click', function () {
						const $inside = $( this )
							.closest( '.postbox' )
							.find( '.inside' );
		...
		function _renderGeneral() {
			...
			// CPT selector
			const $cptSel = _cptSelect( s.cpt, ( v ) => {
				s.cpt = v;
				_save();
			} );
			...
			const $statusSel = $( '<select>' )
				.append(
					[ 'publish', 'draft', 'private', 'pending' ].map( ( st ) =>
						$( '<option>' )
							.val( st )
							.text( st )
							.prop( 'selected', st === s.post_status )
					)
				)
				.on( 'change', function () {
					s.post_status = $( this ).val();
					_save();
				} );
			...
			const $updateChk = $(
				'<input type="checkbox" id="wrm_update_existing">'
			).prop( 'checked', s.update_existing );
			const $matchBlock = $( '<div class="wrm-match-block">' )
				.toggle( s.update_existing )
				.append(
					_frow(
						'Match source path',
						$(
							'<input type="text" class="widefat" placeholder="e.g. email">'
						)
							.val( s.match_source )
							.on( 'input', function () {
								s.match_source = $( this ).val();
								_save();
							} ),
						'Dot-path in payload to match against'
					),
					_frow(
						'Match post field',
						$(
							'<input type="text" class="widefat" placeholder="post_title or meta:user_email">'
						)
							.val( s.match_field )
							.on( 'input', function () {
								s.match_field = $( this ).val();
								_save();
							} )
					)
				);
			$updateChk.on( 'change', function () {
				s.update_existing = $( this ).is( ':checked' );
				$matchBlock.slideToggle( 150 );
				_save();
			} );
		...
		function _renderFields() {
			...
			_state.fields.forEach( ( f, idx ) =>
				$list.append( _fieldRow( f, idx ) )
			);

			const $add = $(
				'<button type="button" class="button wrm-btn-add">+ Add Field Mapping</button>'
			);
			$add.on( 'click', function () {
				const f = {
					source: '',
			...
			return $( '<div class="wrm-fields-wrap">' )
				.append( $list )
				.append( $add );
		}

		function _fieldRow( field, idx ) {
			const $row = $( '<div class="wrm-field-row">' ).attr(
				'data-idx',
				idx
			);
			...
			// Drag handle
			$row.append(
				'<span class="wrm-drag dashicons dashicons-move" draggable="true"></span>'
			);

			// Source cell
			const $srcInp = $(
				'<input type="text" class="widefat" placeholder="payload.field or {{tag}}">'
			).val( field.source );
			$srcInp.on( 'input', function () {
				field.source = $( this ).val();
				_save();
			} );
			const $srcTag = $(
				'<button type="button" class="button-link wrm-tag-btn" title="' +
					esc( t( 'insertTag', 'Insert tag' ) ) +
					'">◉</button>'
			);
			$srcTag.on( 'click', function () {
				WRM.Tags.showPicker( $srcInp[ 0 ], $srcInp );
			} );
			...
			const $loopChk = $( '<input type="checkbox">' ).prop(
				'checked',
				!! field.loop
			);
			const $loopOpts = $( '<div class="wrm-loop-opts">' )
				.toggle( !! field.loop )
				.append(
					_frow(
						'Loop type',
						$( '<select>' )
							...
							.val( field.loop_type || 'join' )
							.on( 'change', function () {
								field.loop_type = $( this ).val();
					...
					_frow(
						'Item template',
						$(
							'<input type="text" class="widefat" placeholder="{{item}} or {{item.name}}">'
						)
							.val( field.loop_template || '{{item}}' )
							.on( 'input', function () {
								field.loop_template = $( this ).val();
								_save();
							} )
					),
					_frow(
						'Separator',
						$(
							'<input type="text" class="short-text" placeholder=",">'
						)
							.val( field.loop_join || ',' )
							.on( 'input', function () {
								field.loop_join = $( this ).val();
								_save();
							} )
					)
				);
			const $joinRow = $loopOpts.find( '.wrm-form-row:last' );
			$joinRow.toggle( field.loop_type === 'join' );
			$loopChk.on( 'change', function () {
				field.loop = $( this ).is( ':checked' );
				$loopOpts.slideToggle( 150 );
				_save();
			} );
			$srcCell.append(
				$( '<label class="wrm-loop-label">' ).append(
					$loopChk,
					' Loop (source is array)'
				),
				$loopOpts
			);
			...
			// Remove button
			const $del = $(
				'<button type="button" class="button-link wrm-del-row" title="Remove">✕</button>'
			);
			$del.on( 'click', function () {
				const i = _state.fields.indexOf( field );
				if ( i !== -1 ) {
					_state.fields.splice( i, 1 );
				}
		...
		function _targetSelector( field ) {
			let tType = 'post_field',
				tKey = field.target || 'post_title';
			if ( tKey.startsWith( 'meta_json:' ) ) {
				tType = 'meta_json';
				tKey = tKey.slice( 10 );
			} else if ( tKey.startsWith( 'meta:' ) ) {
				tType = 'meta';
				tKey = tKey.slice( 5 );
			} else if ( tKey.startsWith( 'taxonomy:' ) ) {
				tType = 'taxonomy';
				tKey = tKey.slice( 9 );
			}
			...
			const $typesel = $( '<select class="wrm-tgt-type">' )
				.append(
			...
			const $pfsel = $( '<select class="wrm-pf-sel">' )
				.append(
					POST_FIELDS.map( ( f ) =>
						$( '<option>' )
							.val( f )
							.text( f )
							.prop( 'selected', f === tKey )
					)
				)
				.toggle( tType === 'post_field' );
			const $keyinp = $(
				'<input type="text" class="wrm-key-inp widefat" placeholder="key name">'
			)
				.val( tKey )
				.toggle( tType !== 'post_field' );
			...
			$keyinp.on( 'blur', function () {
				if ( $typesel.val() !== 'post_field' ) {
					$( this ).val( sanitizeKey( $( this ).val() ) );
				}
			} );
		...
		function _renderChains() {
			...
			[
				...
				$addRow.append(
					$(
						'<button type="button" class="button wrm-add-chain-btn">'
					)
			...
			return $( '<div class="wrm-chains-wrap">' )
				.append( $list )
				.append( $addRow );
		}
		...
		function _chainItem( chain ) {
			...
			const $item = $( '<div class="wrm-chain-item">' ).attr(
				'data-uid',
				chain._uid
			);
			...
			const $hdr = $( '<div class="wrm-chain-hdr">' ).append(
				...
				$(
					'<span class="wrm-chain-badge wrm-chain-' +
						esc( chain.type ) +
						'">'
				).text( ( icons[ chain.type ] || '' ) + ' ' + chain.type ),
				$(
					'<button type="button" class="button-link wrm-chain-tog">▾</button>'
				).on( 'click', function () {
					$body.slideToggle( 150 );
					$( this ).text( $body.is( ':visible' ) ? '▾' : '▸' );
				} ),
				$(
					'<button type="button" class="button-link wrm-chain-del" title="Remove">✕</button>'
				).on( 'click', function () {
					const i = _state.chains.indexOf( chain );
					if ( i !== -1 ) {
						_state.chains.splice( i, 1 );
					}
			...
			const renderers = {
				webhook: _chainWebhook,
				dispatcher: _chainWebhook,
				email: _chainEmail,
				sms: _chainSms,
				action: _chainAction,
				function: _chainFunction,
				mapping: _chainMapping,
			};
			( renderers[ chain.type ] || ( () => $( '<p>Unknown type</p>' ) ) )(
				chain,
				$body
			);
		...
		function _chainWebhook( chain, $body ) {
			$body.append(
				_frow(
					'URL',
					$(
						'<input type="url" class="widefat" placeholder="https://api.example.com/webhook">'
					)
						.val( chain.url || '' )
						.on( 'input', function () {
							chain.url = $( this ).val();
				...
				_frow(
					'Timeout (s)',
					$(
						'<input type="number" min="1" max="60" class="small-text">'
					)
						.val( chain.timeout || 15 )
						.on( 'input', function () {
							chain.timeout =
								parseInt( $( this ).val(), 10 ) || 15;
							_save();
						} )
				),
				_frow(
					'Signing Secret',
					$(
						'<input type="text" class="widefat" placeholder="Leave blank to skip outbound HMAC signing">'
					)
						.val( chain.signing_secret || '' )
						.on( 'input', function () {
							chain.signing_secret = $( this ).val();
							_save();
						} ),
			...
			function _renderHeaders() {
				...
				Object.entries( chain.headers || {} ).forEach( ( [ k, v ] ) => {
					const $kr = $(
						'<input type="text" class="wrm-hdr-key" placeholder="Header-Name">'
					).val( k );
					const $vr = $(
						'<input type="text" class="wrm-hdr-val" placeholder="value or {{tag}}">'
					).val( v );
					const $dr = $(
					...
					$vr.on( 'input', function () {
						chain.headers[ $kr.val() ] = $( this ).val();
						_save();
					} );
					$hdrList.append(
						$( '<div class="wrm-hdr-row">' ).append(
							$kr,
							$( '<span>:</span>' ),
							$vr,
							$dr
			...
			_renderHeaders();
			const $addHdr = $(
				'<button type="button" class="button-link">+ Add header</button>'
			).on( 'click', function () {
				if ( ! chain.headers ) {
					chain.headers = {};
				}
			...
			$body.append(
				_frow( 'Headers', $( '<div>' ).append( $hdrList, $addHdr ) )
			);
			...
			WRM.BodyBuilder.render( $bbContainer, bbRoot, function () {
				chain.body_builder = JSON.parse(
					WRM.BodyBuilder.toJSON( bbRoot )
				);
				_save();
			} );
			$body.append(
		...
		function _tagInput( chain, key, placeholder ) {
			const $inp = $( '<input type="text" class="widefat">' )
				.val( chain[ key ] || '' )
			...
			const $tag = $(
				'<button type="button" class="button-link wrm-tag-btn" title="Insert merge tag">◉</button>'
			);
		...
		function _chainEmail( chain, $body ) {
			...
			const $bodyRow = _frow(
				...
				( function () {
					const $ta = $(
						'<textarea class="widefat" rows="6" placeholder="Hi {{payload.name}}, ...">'
					).val( chain.body_template || '' );
					$ta.on( 'input', function () {
						chain.body_template = $( this ).val();
						_save();
					} );
					return $ta;
				} )(),
			...
			const $mjmlRow = _frow(
				...
				( function () {
					const $ta = $(
						'<textarea class="widefat code" rows="10" placeholder="<mjml><mj-body><mj-section><mj-column><mj-text>Hi {{payload.name}}</mj-text></mj-column></mj-section></mj-body></mjml>">'
					).val( chain.mjml_source || '' );
					$ta.on( 'input', function () {
						chain.mjml_source = $( this ).val();
						_save();
					} );
					return $ta;
				} )(),
			...
			$body.append(
				_frow(
					...
					$( '<select>' )
						...
						.val( fmt )
						.on( 'change', function () {
							chain.format = $( this ).val();
							applyFormat( chain.format );
							_save();
						} )
				)
			);
			...
			$body.append(
				...
				_frow(
					...
					$( '<label class="wrm-toggle">' ).append(
						$( '<input type="checkbox">' )
							.prop( 'checked', chain.track !== false )
							.on( 'change', function () {
								chain.track = $( this ).is( ':checked' );
								_save();
							} ),
		...
		const SMS_PROVIDERS = {
			twilio: {
				label: 'Twilio',
				fields: [
					[ 'account_sid', 'Account SID', 'ACxxxx', 'text' ],
					[ 'auth_token', 'Auth Token', '', 'password' ],
					[ 'from', 'From number', '+15551234567', 'text' ],
				],
			},
			whatsapp: {
				...
				fields: [
					...
					[ 'api_version', 'Graph API version', 'v19.0', 'text' ],
				],
			},
			sinch: {
				label: 'Sinch',
				fields: [
					[ 'service_plan_id', 'Service Plan ID', '', 'text' ],
		...
		function _chainSms( chain, $body ) {
			$body.append(
				_frow(
					...
					$( '<select>' )
						.append(
							Object.entries( SMS_PROVIDERS ).map( ( [ k, v ] ) =>
								$( '<option>' ).val( k ).text( v.label )
							)
						)
				...
				_frow(
					...
					( function () {
						const $ta = $(
							'<textarea class="widefat" rows="3" placeholder="Hi {{payload.name}}, ...">'
						).val( chain.body || '' );
						$ta.on( 'input', function () {
							chain.body = $( this ).val();
							_save();
			...
			function renderProviderFields() {
				$creds.empty();
				const def =
					SMS_PROVIDERS[ chain.provider || 'twilio' ] ||
					SMS_PROVIDERS.twilio;
				def.fields.forEach( ( [ key, label, ph, type ] ) => {
					$creds.append(
						_frow( label, _textInput( chain, key, ph, type ) )
					);
				} );
			}
			renderProviderFields();
		...
		function _chainAction( chain, $body ) {
			$body.append(
				_frow(
					'WP Action Hook',
					$(
						'<input type="text" class="widefat" placeholder="my_hook_name">'
					)
						.val( chain.hook || '' )
						.on( 'input', function () {
							chain.hook = $( this ).val();
		...
		function _chainFunction( chain, $body ) {
			$body.append(
				_frow(
					'PHP Function Name',
					$(
						'<input type="text" class="widefat" placeholder="my_callback_function">'
					)
						.val( chain.function || '' )
						.on( 'input', function () {
							chain.function = $( this ).val();
							_save();
						} ),
		...
		function _chainMapping( chain, $body ) {
			const $sel = $( '<select class="widefat">' ).append(
			...
			apiFetch( { path: '/wrm/v1/mappings?per_page=100' } )
					...
					maps.forEach( ( m ) =>
						$sel.append(
							$( '<option>' )
								...
								.prop(
									'selected',
									parseInt( m.id ) === chain.mapping_id
								)
						)
					);
				} )
				.catch( () => {} );
			$sel.on( 'change', function () {
				chain.mapping_id = parseInt( $( this ).val(), 10 );
				_save();
			} );

			const $srcSel = $( '<select class="widefat">' )
				.append(
			...
			const $keyInp = $(
				'<input type="text" class="widefat" placeholder="meta key containing JSON">'
			).val( chain.chain_source_key || '' );
			const $keyRow = _frow(
				'Meta key',
				$keyInp,
				'The post meta key written by a <code>meta_json:</code> target — its JSON value becomes the chained payload.'
			);
			$keyRow.toggle(
				( chain.chain_source || 'payload' ) === 'post_meta_json'
			);
			...
			$keyInp.on( 'input', function () {
				chain.chain_source_key = sanitizeKey( $( this ).val() );
				_save();
			} );
			$keyInp.on( 'blur', function () {
				$( this ).val( chain.chain_source_key );
			} );
			...
			$body.append(
				_frow(
					'Target Mapping',
					$sel,
					'Chains the result through another mapping.'
				),
				_frow(
					'Chain Source',
					$srcSel,
					'What payload to pass into the chained mapping.'
				),
				$keyRow
			);
		}
		...
		function _renderTagsPanel() {
			const $cloud = $(
				'<div id="wrm-available-tags" class="wrm-tags-cloud">'
			);
			...
			if ( WRM.Tags._loaded ) {
				...
				$cloud.html(
					'<em class="wrm-muted">' +
						esc( t( 'noCaptures', 'No captures yet.' ) ) +
						'</em>'
				);
			}
			return $( '<div>' ).append(
		...
		function _renderPreview() {
			const $pre = $( '<pre class="wrm-config-pre code">' ).text(
		...
		function _frow( label, $ctrl, desc ) {
			...
			if ( desc ) {
				$row.find( '.wrm-form-ctrl' ).append(
					$( '<p class="description">' ).html( desc )
				);
			}
			return $row;
		}
		...
		function _cptSelect( current, onChange ) {
			...
			const pts =
				window.wrmData && window.wrmData.postTypes
					? window.wrmData.postTypes
					: [];
			if ( pts.length ) {
				pts.forEach( ( pt ) =>
					$sel.append(
	...
	$( function () {
		...
		$( document ).on( 'dragstart', '.wrm-drag', function ( e ) {
			_dragSrc = $( this ).closest(
				'.wrm-field-row, .wrm-chain-item'
			)[ 0 ];
			if ( _dragSrc ) {
