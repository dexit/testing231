 ...
 * Description:       API gateway for WordPress: ingest webhooks, transform payloads, chain actions (email/SMS/webhook/function), and track outcomes with async queuing and retry.
 * Version:           1.3.0
...
add_action(
	...
	static function (): void {
		...
		WRM_Scheduler::init();
		WRM_Admin_API::init();
		if ( class_exists( 'Elementor\Plugin' ) && class_exists( 'WRM_Elementor' ) ) {
			WRM_Elementor::init();
		}
		if ( is_admin() ) {
			WRM_Admin::init();
