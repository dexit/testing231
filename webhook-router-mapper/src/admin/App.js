...
export default function App() {
	const initialTab = window.wrmAdminData?.initialTab || 'start';
	const tabs = [
		{ name: 'start', title: '✦ Get Started', className: 'tab-start' },
				...
				<TabPanel tabs={ tabs } initialTabName={ initialTab }>
					{ ( tab ) => (
						<ErrorBoundary key={ tab.name }>
							{ tab.name === 'start' && <GettingStartedPage /> }
