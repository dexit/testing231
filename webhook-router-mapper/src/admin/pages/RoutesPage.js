...
export default function RoutesPage() {
	...
	const handleSave = () => {
		...
		apiFetch( { path: '/wrm/v1/routes', method: 'POST', data: form } )
			...
			.catch( ( e ) => {
				setSaving( false );
				showNotice( e.message || 'Save failed', 'error' );
			} );
	};
	...
	const handleToggleStatus = ( route ) => {
		...
		apiFetch( {
			path: `/wrm/v1/routes/${ route.slug }`,
			method: 'PUT',
			data: { status: newStatus },
		} )
			.then( () => {
				showNotice( `Route ${ newStatus }.` );
				loadRoutes();
			} )
			.catch( ( e ) =>
	...
	const handleDelete = () => {
		...
		apiFetch( {
			path: `/wrm/v1/routes/${ deleteTarget.slug }`,
			method: 'DELETE',
		} )
			.then( () => {
	...
	const handleEditSave = () => {
		setEditSaving( true );
		apiFetch( {
			path: `/wrm/v1/routes/${ editTarget.slug }`,
			method: 'PUT',
			data: editForm,
		} )
			.then( () => {
	...
	const copyWebhookUrl = ( route ) => {
		const url = `${ window.wrmAdminData?.siteUrl || '' }/webhook/${
			route.slug
		}`;
		navigator.clipboard
			?.writeText( url )
	...
	return (
		<div className="wrm-page wrm-routes-page">
				...
				>
					...
					>
						...
						<TextControl
							...
							onChange={ ( v ) =>
								setForm( ( f ) => ( { ...f, rate_limit: v } ) )
							}
							type="number"
						/>
						<TextControl
							label="Rate Window (s)"
							value={ form.rate_window }
							onChange={ ( v ) =>
								setForm( ( f ) => ( { ...f, rate_window: v } ) )
							}
							type="number"
						/>
						<SelectControl
			...
			{ deleteTarget && (
				...
				>
					<p>
						Delete route <strong>{ deleteTarget.slug }</strong>?
			...
			{ editTarget && (
				...
				>
					...
					{ rateUsage && rateUsage.rate_limit > 0 && (
						...
						>
							<strong>Rate Usage:</strong> { rateUsage.used } /{ ' ' }
							{ rateUsage.rate_limit } requests in current{ ' ' }
							{ rateUsage.rate_window }s window
							<div
								style={ {
									height: 6,
									background: '#e2e4e7',
							...
							{ rateUsage.window_reset_at && (
								...
								>
									Resets at { rateUsage.window_reset_at } UTC
								</span>
							) }
						</div>
					) }
