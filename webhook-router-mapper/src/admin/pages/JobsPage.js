...
export default function JobsPage() {
	...
	const handleRetry = ( jobId ) => {
		apiFetch( { path: `/wrm/v1/jobs/${ jobId }/retry`, method: 'POST' } )
			.then( () => {
				showNotice( `Job #${ jobId } queued for retry.` );
				loadJobs();
				loadStats();
			} )
			.catch( ( e ) =>
				showNotice( e.message || 'Retry failed', 'error' )
			);
	};
	...
	const handleBulkRetry = () => {
		Promise.all(
			selected.map( ( id ) =>
				apiFetch( {
					path: `/wrm/v1/jobs/${ id }/retry`,
					method: 'POST',
				} )
			)
		)
			.then( () => {
				showNotice( `${ selected.length } job(s) queued for retry.` );
				setSelected( [] );
				loadJobs();
				loadStats();
			} )
			.catch( ( e ) =>
				showNotice( e.message || 'Bulk retry failed', 'error' )
			);
	};
	...
	const retryableSelected = selected.filter( ( id ) => {
		const jb = jobs.find( ( job ) => job.id === id );
		return jb && ( jb.status === 'failed' || jb.status === 'dead' );
	} );
