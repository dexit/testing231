...
export default function LogsPage() {
	...
	const loadLogs = useCallback( () => {
		...
		if ( filterContext ) {
			params.set( 'context', filterContext );
		}
		if ( search ) {
	...
	useEffect( () => {
		...
		const fetchTail = () => {
			apiFetch( {
				path: `/wrm/v1/logs/tail?since_id=${ lastIdRef.current }&limit=50`,
			} )
				.then( ( rows ) => {
					if ( Array.isArray( rows ) && rows.length > 0 ) {
						lastIdRef.current = rows[ rows.length - 1 ].id;
						setTailLogs( ( prev ) =>
							[ ...rows, ...prev ].slice( 0, 200 )
						);
					}
				} )
				.catch( () => {} );
			...
			{ showPurgeConfirm && (
				...
				>
					<p>
						...
						<strong>all logs</strong>? This cannot be undone.
					</p>
					<div
						style={ {
							display: 'flex',
