...
import { Button, SelectControl, Notice, Spinner, Modal, TextareaControl } from '@wordpress/components';
...
export default function CapturesPage() {
  ...
  const handleInspect = (capture) => {
    ...
    apiFetch({ path: `/wrm/v1/captures/${capture.id}/paths` })
      .then(data => { setPaths(Array.isArray(data) ? data : (data.paths || [])); setPathsLoading(false); })
  ...
  const handleReplay = () => {
    if (!inspecting || !replayMappingId) return;
    setReplaying(true);
    setReplayResult(null);
    apiFetch({ path: `/wrm/v1/captures/${inspecting.id}/replay`, method: 'POST', data: { mapping_id: +replayMappingId } })
      .then(result => { setReplayResult({ ok: true, data: result }); setReplaying(false); })
      .catch(e => { setReplayResult({ ok: false, msg: e.message || 'Replay failed' }); setReplaying(false); });
  };

  // Open edit-and-resubmit modal
  const handleOpenResubmit = () => {
    const pretty = JSON.stringify(parsePayload(inspecting?.payload), null, 2);
  ...
  const handleResubmit = () => {
    if (!inspecting || !resubmitMappingId) return;
    let payload;
    try {
      payload = JSON.parse(resubmitPayload);
      if (typeof payload !== 'object' || Array.isArray(payload) || payload === null) throw new Error('Must be a JSON object');
      setResubmitPayloadError('');
    } catch (e) {
    ...
    apiFetch({
      ...
      .then(result => { setResubmitResult({ ok: true, data: result }); setResubmitting(false); loadCaptures(); })
      .catch(e => { setResubmitResult({ ok: false, msg: e.message || 'Resubmit failed' }); setResubmitting(false); });
  };
  ...
  return (
    <div className="wrm-page wrm-captures-page">
      ...
      <div style={{ display: 'flex', gap: 16, alignItems: 'flex-end', marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        />
        <Button variant="secondary" onClick={() => { setPage(1); loadCaptures(); }}>Refresh</Button>
      </div>
      ...
      {loading ? <Spinner /> : error ? (
        ...
        <>
            ...
            <tbody>
              ...
              {captures.map(cap => (
                <tr key={cap.id}>
                  ...
                  <td>
                    <Button variant="secondary" isSmall onClick={() => handleInspect(cap)}>Inspect</Button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 12 }}>
            ...
            <Button variant="secondary" isSmall disabled={page >= totalPages} onClick={() => setPage(p => p + 1)}>Next &rarr;</Button>
          </div>
        </>
      )}
      ...
      {inspecting && (
        <Modal
          title={`Capture #${inspecting.id} — ${inspecting.route_slug || ''}`}
          onRequestClose={() => { setInspecting(null); setResubmitOpen(false); }}
        ...
        >
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, minHeight: 320 }}>
            ...
            <div>
              ...
              <div style={{ background: '#f8f9fa', border: '1px solid #e2e4e7', borderRadius: 4, padding: 12, overflowY: 'auto', maxHeight: 380, fontFamily: 'monospace', fontSize: 12 }}>
                <JsonTree data={parsePayload(inspecting.payload)} />
              </div>

              <h4 style={{ fontSize: 13, marginBottom: 6, marginTop: 14 }}>Available Paths</h4>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              {/* 1 — Synchronous Apply */}
              <div style={{ padding: 14, border: '1px solid #e2e4e7', borderRadius: 4 }}>
                <h3 style={{ marginTop: 0, fontSize: 13, marginBottom: 10 }}>Apply Mapping <span style={{ fontWeight: 400, color: '#888', fontSize: 11 }}>(synchronous)</span></h3>
                <SelectControl label="Mapping" value={applyMappingId} options={mappingOptions} onChange={v => { setApplyMappingId(v); setReplayMappingId(v); }} />
                <Button variant="primary" onClick={handleApplyMapping} disabled={!applyMappingId || applying} style={{ marginTop: 4 }}>
                  {applying ? <Spinner /> : 'Apply Now'}
                </Button>
                {applyResult && (
                  <div style={{ marginTop: 10 }}>
                    {applyResult.ok
                      ? <Notice status="success" isDismissible={false}>Applied — post #{applyResult.data?.post_id || '?'}</Notice>
              ...
              {/* 2 — Async Replay */}
              <div style={{ padding: 14, border: '1px solid #e2e4e7', borderRadius: 4 }}>
                <h3 style={{ marginTop: 0, fontSize: 13, marginBottom: 10 }}>Replay via Job Queue <span style={{ fontWeight: 400, color: '#888', fontSize: 11 }}>(async)</span></h3>
                <SelectControl label="Mapping" value={replayMappingId} options={mappingOptions} onChange={v => { setReplayMappingId(v); setApplyMappingId(v); }} />
                <Button variant="secondary" onClick={handleReplay} disabled={!replayMappingId || replaying} style={{ marginTop: 4 }}>
                  {replaying ? <Spinner /> : '↻ Enqueue Replay'}
                </Button>
                {replayResult && (
                  <div style={{ marginTop: 10 }}>
                    {replayResult.ok
                      ? <Notice status="success" isDismissible={false}>Job #{replayResult.data?.job_id} queued. Check the Jobs tab.</Notice>
                      : <Notice status="error" isDismissible={false}>{replayResult.msg}</Notice>}
                  </div>
                )}
              </div>
              ...
              {/* 3 — Edit & Resubmit */}
              <div style={{ padding: 14, border: '1px solid #e2e4e7', borderRadius: 4 }}>
                <h3 style={{ marginTop: 0, fontSize: 13, marginBottom: 6 }}>Edit &amp; Resubmit <span style={{ fontWeight: 400, color: '#888', fontSize: 11 }}>(new capture + async)</span></h3>
                <p style={{ fontSize: 12, color: '#666', margin: '0 0 10px' }}>
                  Opens an editor to modify the payload before re-running. Stores a new capture so the original is preserved.
                </p>
                <Button variant="tertiary" onClick={handleOpenResubmit}>✏ Open Payload Editor</Button>
              </div>
            </div>
          </div>
          ...
          {resubmitOpen && (
            <div style={{ marginTop: 24, padding: 16, border: '2px solid #007cba', borderRadius: 4, background: '#f0f6fc' }}>
              <h3 style={{ marginTop: 0, fontSize: 14 }}>Edit Payload &amp; Resubmit</h3>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 16, alignItems: 'flex-start' }}>
                <div>
                  <TextareaControl
                    label="Payload JSON"
                    value={resubmitPayload}
                    onChange={v => { setResubmitPayload(v); setResubmitPayloadError(''); }}
                    rows={12}
                    style={{ fontFamily: 'monospace', fontSize: 12 }}
                  />
                  {resubmitPayloadError && (
                    <p style={{ color: '#d63638', fontSize: 12, margin: '4px 0 0' }}>{resubmitPayloadError}</p>
                  )}
                </div>
                <div style={{ minWidth: 200 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                    <Button variant="primary" onClick={handleResubmit} disabled={!resubmitMappingId || resubmitting}>
                      {resubmitting ? <Spinner /> : '↺ Resubmit'}
                    </Button>
                    <Button variant="tertiary" onClick={() => { setResubmitOpen(false); setResubmitResult(null); }}>Cancel</Button>
                  </div>
                  {resubmitResult && (
                    <div style={{ marginTop: 10 }}>
                      {resubmitResult.ok
                        ? <Notice status="success" isDismissible={false}>
                            New capture #{resubmitResult.data?.new_capture_id}, job #{resubmitResult.data?.job_id} queued.
                          </Notice>
                        : <Notice status="error" isDismissible={false}>{resubmitResult.msg}</Notice>}
                    </div>
                  )}
                </div>
              </div>
