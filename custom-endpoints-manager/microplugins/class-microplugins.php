...
class Microplugins {
    ...
    protected function __construct() {
        ...
        if ( true === self::check_file_permissions() ) {
            ...
            $message = sprintf(
                __( 'Microplugins needs read and write permissions on the "%s" directory. Current permissions: %s.', 'custom-endpoints-manager' ),
        ...
        <div class="wrap">
            ...
            if ( empty( $files ) ) {
                ...
                foreach ( $files as $filename ) {
                    ...
                    printf( '<p>%s %s</p>', esc_html( sprintf( __( 'Deleting %s...', 'custom-endpoints-manager' ), basename( $filename ) ) ), esc_html( $result ) );
                }
            }
            ?>
            ...
            if ( empty( $posts ) ) {
                ...
                foreach ( $posts as $post ) {
                    $result = self::dump( $post->ID )
                        ? esc_html__( 'OK', 'custom-endpoints-manager' )
                        : esc_html__( 'ERROR', 'custom-endpoints-manager' );
                    printf( '<p>%s %s</p>', esc_html( sprintf( __( 'Generating microplugin %d...', 'custom-endpoints-manager' ), intval( $post->ID ) ) ), esc_html( $result ) );
                }
            }
            ?>
        </div>
        ...
        if ( $post->post_status === 'publish' ) {
            ...
            if ( ! $validation['valid'] ) {
                ...
                self::add_admin_notice(
                    sprintf(
                        __( 'Microplugin #%d validation failed and was not published: %s', 'custom-endpoints-manager' ),
                        $post_id,
    ...
    public static function shutdown_function() {
        ...
        if ( true === self::is_microplugin_error( $last_error ) ) {
            $error_filename = MICROPLUGINS_CACHE_DIR . '/error';
            file_put_contents( $error_filename, json_encode( $last_error ) );

            if ( ! headers_sent() ) {
                $referer = wp_get_referer();
                wp_redirect( $referer ? add_query_arg( 'cem_error', 'microplugin_fatal', $referer ) : admin_url() );
                exit;
            }
        }
    }
    ...
    public static function process_mircoplugin_error( $error ) {
        ...
        if ( $is_fatal ) {
            $message = sprintf(
                __( 'The microplugin #%d "%s" has been disabled due to fatal errors. <a href="%s">Edit microplugin</a>', 'custom-endpoints-manager' ),
                $post->ID, $post->post_title, $edit_link
            );
            self::add_admin_notice( $message, 'error' );
        } else {
            $message = sprintf(
                __( 'The microplugin #%d "%s" has warnings. <a href="%s">Edit microplugin</a>', 'custom-endpoints-manager' ),
                $post->ID, $post->post_title, $edit_link
            );
            self::add_admin_notice( $message, 'warning' );
        }
    ...
    public static function activation_hook() {
        ...
        $capabilities = array(
            ...
            'edit_private_microplugins', 'edit_published_microplugins', 'publish_microplugins',
    ...
    protected static function get_ace_themes() {
        $bright = array(
            'chrome'         => array( 'id' => 'chrome', 'name' => 'Chrome' ),
            'clouds'         => array( 'id' => 'clouds', 'name' => 'Clouds' ),
            'crimson_editor' => array( 'id' => 'crimson_editor', 'name' => 'Crimson Editor' ),
            'dawn'           => array( 'id' => 'dawn', 'name' => 'Dawn' ),
            'dreamweaver'    => array( 'id' => 'dreamweaver', 'name' => 'Dreamweaver' ),
            'eclipse'        => array( 'id' => 'eclipse', 'name' => 'Eclipse' ),
            'github'         => array( 'id' => 'github', 'name' => 'GitHub' ),
            'iplastic'       => array( 'id' => 'iplastic', 'name' => 'IPlastic' ),
            'solarized_light'=> array( 'id' => 'solarized_light', 'name' => 'Solarized Light' ),
            'textmate'       => array( 'id' => 'textmate', 'name' => 'TextMate' ),
            'tomorrow'       => array( 'id' => 'tomorrow', 'name' => 'Tomorrow' ),
            'xcode'          => array( 'id' => 'xcode', 'name' => 'XCode' ),
            'kuroir'         => array( 'id' => 'kuroir', 'name' => 'Kuroir' ),
            'katzenmilch'    => array( 'id' => 'katzenmilch', 'name' => 'KatzenMilch' ),
            'sqlserver'      => array( 'id' => 'sqlserver', 'name' => 'SQL Server' ),
        );

        $dark = array(
            'ambiance'               => array( 'id' => 'ambiance', 'name' => 'Ambiance' ),
            'chaos'                  => array( 'id' => 'chaos', 'name' => 'Chaos' ),
            'clouds_midnight'        => array( 'id' => 'clouds_midnight', 'name' => 'Clouds Midnight' ),
            'cobalt'                 => array( 'id' => 'cobalt', 'name' => 'Cobalt' ),
            'idle_fingers'           => array( 'id' => 'idle_fingers', 'name' => 'idle Fingers' ),
            'kr_theme'               => array( 'id' => 'kr_theme', 'name' => 'krTheme' ),
            'merbivore'              => array( 'id' => 'merbivore', 'name' => 'Merbivore' ),
            'merbivore_soft'         => array( 'id' => 'merbivore_soft', 'name' => 'Merbivore Soft' ),
            'mono_industrial'        => array( 'id' => 'mono_industrial', 'name' => 'Mono Industrial' ),
            'monokai'                => array( 'id' => 'monokai', 'name' => 'Monokai' ),
            'pastel_on_dark'         => array( 'id' => 'pastel_on_dark', 'name' => 'Pastel on dark' ),
            'solarized_dark'         => array( 'id' => 'solarized_dark', 'name' => 'Solarized Dark' ),
            'terminal'               => array( 'id' => 'terminal', 'name' => 'Terminal' ),
            'tomorrow_night'         => array( 'id' => 'tomorrow_night', 'name' => 'Tomorrow Night' ),
            'tomorrow_night_blue'    => array( 'id' => 'tomorrow_night_blue', 'name' => 'Tomorrow Night Blue' ),
            'tomorrow_night_bright'  => array( 'id' => 'tomorrow_night_bright', 'name' => 'Tomorrow Night Bright' ),
            'tomorrow_night_eighties'=> array( 'id' => 'tomorrow_night_eighties', 'name' => 'Tomorrow Night 80s' ),
            'twilight'               => array( 'id' => 'twilight', 'name' => 'Twilight' ),
            'vibrant_ink'            => array( 'id' => 'vibrant_ink', 'name' => 'Vibrant Ink' ),
        );

        return array( 'bright' => $bright, 'dark' => $dark );
    }
