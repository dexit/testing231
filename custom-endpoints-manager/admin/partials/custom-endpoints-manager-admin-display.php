...
$active_tab        = isset( $_GET['tab'] ) ? sanitize_text_field( $_GET['tab'] ) : 'endpoints';

$microplugins_posts = get_posts( array(
...
<div class="wrap">
    ...
    <nav class="nav-tab-wrapper">
        <a href="?page=custom-endpoints-manager&tab=endpoints"
           class="nav-tab <?php echo $active_tab === 'endpoints' ? 'nav-tab-active' : ''; ?>">
    ...
    <?php if ( $active_tab === 'endpoints' ) : ?>
        ...
        <form method="post" action="<?php echo esc_url( admin_url( 'options-general.php?page=custom-endpoints-manager' ) ); ?>">
            ...
            <table class="widefat fixed striped" id="cem-endpoints-table">
                ...
                <tbody id="custom-endpoints-list">
                    <?php
                    $rows = ! empty( $custom_endpoints ) ? $custom_endpoints : array( array( 'slug' => '', 'methods' => '', 'capability' => '', 'microplugin_id' => 0, 'args' => '' ) );
                    foreach ( $rows as $index => $endpoint ) :
                        $selected_mp_id = isset( $endpoint['microplugin_id'] ) ? intval( $endpoint['microplugin_id'] ) : 0;
                    ?>
                    <tr>
                        <td>
                            <input type="text" name="cem_endpoints[<?php echo $index; ?>][slug]"
                                value="<?php echo esc_attr( $endpoint['slug'] ); ?>" class="regular-text" />
                        </td>
                        <td>
                            <input type="text" name="cem_endpoints[<?php echo $index; ?>][methods]"
                                value="<?php echo esc_attr( $endpoint['methods'] ); ?>" class="small-text"
                                placeholder="GET" />
                        </td>
                        <td>
                            <input type="text" name="cem_endpoints[<?php echo $index; ?>][capability]"
                                value="<?php echo esc_attr( $endpoint['capability'] ); ?>" class="regular-text"
                                placeholder="read" />
                        </td>
                        <td>
                            <select name="cem_endpoints[<?php echo $index; ?>][microplugin_id]">
                            ...
                            <?php if ( $selected_mp_id ) : ?>
                                <p class="description">
                                    <?php printf( esc_html__( 'Callback: %s', 'custom-endpoints-manager' ), '<code>cem_microplugin_callback_' . $selected_mp_id . '</code>' ); ?>
                                </p>
                            <?php endif; ?>
                        </td>
                        <td>
        ...
        <ul>
            ...
            $cem_routes  = array_filter( $routes, function( $route ) {
                return strpos( $route, '/cem/v1/' ) !== false;
        ...
        <?php
        // Inline JS data for the "Add Endpoint" button — embed microplugin list as JSON
        $mp_options = array();
        foreach ( $microplugins_posts as $mp_post ) {
            $mp_options[] = array( 'id' => $mp_post->ID, 'title' => $mp_post->post_title . ' (ID: ' . $mp_post->ID . ')' );
        }
