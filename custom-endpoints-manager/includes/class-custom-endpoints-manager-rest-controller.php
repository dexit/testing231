...
/**
 * Registers and handles all CEM REST API routes.
 *
 * @since 1.0.0
 */
class Custom_Endpoints_Manager_REST_Controller {
	...
	/**
	 * Handle a custom endpoint request (sync or async).
	 *
	 * @since 1.0.0
	...
	public function list_jobs( WP_REST_Request $request ): WP_REST_Response {
		...
		$logs = CEM_Execution_Logger::get_logs(
			array(
				'per_page' => min( (int) ( $per_page ? $per_page : 50 ), 100 ),
		...
		return new WP_REST_Response(
			array(
				'jobs'  => $logs,
				'total' => CEM_Execution_Logger::count_logs(),
			),
			200
		);
	}
	...
	/**
	 * Return the CEM function library catalog.
	 *
	 * @since 1.0.0
	 * @param WP_REST_Request $request Incoming request (unused).
	 * @return WP_REST_Response
	 */
	public function get_function_library( WP_REST_Request $request ): WP_REST_Response { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		return new WP_REST_Response(
			array(
	...
	/**
	 * Verify the caller has the required capability for the target endpoint.
	 *
	 * @since 1.0.0
	 * @param WP_REST_Request $request Incoming request.
	 * @return bool
	...
	/**
	 * Parse the args string into a REST API args definition array.
	 *
	 * @since 1.0.0
	 * @param string $args_string Comma-separated "name:type" pairs.
	 * @return array
	 */
